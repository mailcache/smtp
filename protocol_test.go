...
func TestProtocol(t *testing.T) {
	Convey("NewProtocol returns a new Protocol", t, func() {
		...
		So(proto.Hostname, ShouldEqual, "mailcache.example")
		So(proto.Ident, ShouldEqual, "ESMTP MailCache")
	...
	Convey("Start should modify the state correctly", t, func() {
		...
		So(reply.Lines(), ShouldResemble, []string{"220 mailcache.example ESMTP MailCache\r\n"})
	})

	Convey("Modifying the hostname should modify the ident reply", t, func() {
		proto := NewProtocol()
		proto.Ident = "OinkSMTP MailCache"
		reply := proto.Start()
		So(reply, ShouldNotBeNil)
		So(reply, ShouldHaveSameTypeAs, &Reply{})
		So(reply.Status, ShouldEqual, 220)
		So(reply.Lines(), ShouldResemble, []string{"220 mailcache.example OinkSMTP MailCache\r\n"})
	})
	...
	Convey("Modifying the ident should modify the ident reply", t, func() {
		...
		So(reply, ShouldHaveSameTypeAs, &Reply{})
		So(reply.Status, ShouldEqual, 220)
		So(reply.Lines(), ShouldResemble, []string{"220 oink.oink ESMTP MailCache\r\n"})
	})
}
...
func TestProcessCommand(t *testing.T) {
	Convey("ProcessCommand should attempt to process anything", t, func() {
		...
		reply := proto.ProcessCommand("OINK mailcache.example")
		So(proto.State, ShouldEqual, INVALID)
		So(reply, ShouldNotBeNil)
...
func TestParse(t *testing.T) {
	...
	Convey("Parse can call ProcessData", t, func() {
		...
		// FIXME this test relies on mailcache/data, it shouldn't!
...
func TestParseMAIL(t *testing.T) {
	...
	Convey("ParseMAIL should parse MAIL command arguments", t, func() {
		m, err := proto.ParseMAIL("FROM:<oink@mailcache.example>")
		So(err, ShouldBeNil)
		So(m, ShouldEqual, "oink@mailcache.example")
	...
	Convey("ParseMAIL should be case-insensitive", t, func() {
		...
		So(err, ShouldBeNil)
		So(m, ShouldEqual, "oink@mailcache.example")
		m, err = proto.ParseMAIL("FrOm:<oink@oink.mailcache.example>")
		So(err, ShouldBeNil)
		So(m, ShouldEqual, "oink@oink.mailcache.example")
	})
	...
	Convey("ValidateSenderHandler should be called", t, func() {
		...
		proto.ValidateSenderHandler = func(sender string) bool {
			handlerCalled = true
			So(sender, ShouldEqual, "oink@mailcache.example")
			return true
		}
		proto.Start()
		proto.Command(ParseCommand("HELO localhost"))
		So(proto.State, ShouldEqual, MAIL)
		reply := proto.Command(ParseCommand("MAIL From:<oink@mailcache.example>"))
		So(handlerCalled, ShouldBeTrue)
		So(reply, ShouldNotBeNil)
		So(reply.Status, ShouldEqual, 250)
		So(reply.Lines(), ShouldResemble, []string{"250 Sender oink@mailcache.example ok\r\n"})
		So(proto.State, ShouldEqual, RCPT)
	})
	Convey("ValidateSenderHandler errors should be returned", t, func() {
		...
		So(reply, ShouldNotBeNil)
		So(reply.Status, ShouldEqual, 550)
		So(reply.Lines(), ShouldResemble, []string{"550 Invalid sender oink@mailcache.example\r\n"})
		So(proto.State, ShouldEqual, MAIL)
	})
}
...
func TestParseRCPT(t *testing.T) {
	...
	Convey("ParseRCPT should parse RCPT command arguments", t, func() {
		m, err := proto.ParseRCPT("TO:<oink@mailcache.example>")
		So(err, ShouldBeNil)
		So(m, ShouldEqual, "oink@mailcache.example")
	...
	Convey("ParseRCPT should be case-insensitive", t, func() {
		...
		So(err, ShouldBeNil)
		So(m, ShouldEqual, "oink@mailcache.example")
		m, err = proto.ParseRCPT("To:<oink@oink.mailcache.example>")
		So(err, ShouldBeNil)
		So(m, ShouldEqual, "oink@oink.mailcache.example")
	})
	...
	Convey("ValidateRecipientHandler should be called", t, func() {
		...
		proto.ValidateRecipientHandler = func(recipient string) bool {
			handlerCalled = true
			So(recipient, ShouldEqual, "oink@mailcache.example")
			return true
		}
		proto.Start()
		proto.Command(ParseCommand("HELO localhost"))
		proto.Command(ParseCommand("MAIL FROM:<test>"))
		So(proto.State, ShouldEqual, RCPT)
		reply := proto.Command(ParseCommand("RCPT To:<oink@mailcache.example>"))
		So(handlerCalled, ShouldBeTrue)
		So(reply, ShouldNotBeNil)
		So(reply.Status, ShouldEqual, 250)
		So(reply.Lines(), ShouldResemble, []string{"250 Recipient oink@mailcache.example ok\r\n"})
		So(proto.State, ShouldEqual, RCPT)
	})
	Convey("ValidateRecipientHandler errors should be returned", t, func() {
		...
		So(reply, ShouldNotBeNil)
		So(reply.Status, ShouldEqual, 550)
		So(reply.Lines(), ShouldResemble, []string{"550 Invalid recipient oink@mailcache.example\r\n"})
		So(proto.State, ShouldEqual, RCPT)
	})
}
...
func TestAuthPlain(t *testing.T) {
	Convey("Inline AUTH PLAIN should call ValidateAuthenticationHandler", t, func() {
		...
		proto.ValidateAuthenticationHandler = func(mechanism string, args ...string) (*Reply, bool) {
			...
			So(args[0], ShouldEqual, "test@mailcache.example")
